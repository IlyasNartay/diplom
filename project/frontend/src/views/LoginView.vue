...
const t = computed(() => {
  if (language.isRussian.value) {
    return {
      ...
      intro:
        'Используйте учётные данные auth_service: покупка билетов, история и сохранённые карты.',
      email: 'Email',
      password: 'Пароль',
      passwordPh: '••••••••',
      signingIn: 'Входим...',
      ...
      loadError: 'Не удалось войти'
    }
  }
  return {
    eyebrow: 'Sign in',
    title: 'Sign in',
    intro: 'Use your auth_service credentials to buy tickets, view history, and manage saved cards.',
    email: 'Email',
    password: 'Password',
    passwordPh: '••••••••',
...
const footerClass = computed(() =>
  isDark.value ? 'mt-6 text-sm text-sdu-mist/75' : 'mt-6 text-sm text-slate-600'
)

const linkClass = 'text-sdu-copper'
...
<template>
  <section class="mx-auto max-w-xl">
    <div :class="panelClass">
      ...
      <p :class="footerClass">
        {{ t.noAccount }}
        <RouterLink :class="linkClass" to="/register">{{ t.create }}</RouterLink>
      </p>
    </div>
  </section>
</template>
